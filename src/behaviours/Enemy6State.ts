...
export class HurtState extends Enemy6State {
    ...
    constructor(enemy6: Enemy6, previousState: Enemy6State, hurtDirection: string) {
        super(enemy6);
    ...
    enter() {
        ...
        if (this.hurtDirection === 'left') {
            this.enemy6.enemy6Binding!.action = 'rightsufferattack';
        } else {
            this.enemy6.enemy6Binding!.action = 'leftsufferattack';
        }
        ...
        // 减少敌人的血量
        const healthStateMachine = this.enemy6.gameObject.getBehaviour(Enemy6HealthStateMachine);
    ...
    update(duringTime: number) {
        ...
        setTimeout(() => {
            this.enemy6.changeState(this.previousState);
...
export class PatrolState extends Enemy6State {
    ...
    enter() {
        ...
        this.enemy6.gameObject.getBehaviour(RigidBody).b2RigidBody.SetAwake(true); // 确保物理引擎对该对象进行更新
    }
    ...
    update(duringTime: number) {
        ...
        // 移动并检测碰撞
        const velocity = new b2Vec2(this.patrolDirection * this.enemy6.patrolSpeed, rigidBody.b2RigidBody.GetLinearVelocity().y);
        rigidBody.b2RigidBody.SetLinearVelocity(velocity);
        
        if (this.enemy6.enemy6Binding) {
            if (this.patrolDirection > 0) {
                this.enemy6.enemy6Binding.action = 'rightpatrol';
            } else {
                this.enemy6.enemy6Binding.action = 'leftpatrol';
            }
        } else {
            console.error("Enemy6PrefabBinding is not initialized.");
        }
    ...
    handleCollisionEnter(other: RigidBody, otherCollider: Collider, self: RigidBody, selfCollider: Collider) {
        if (selfCollider.tag === 'enemy6body' && otherCollider.tag === 'block') {
            this.patrolDirection *= -1; // 碰到墙壁时改变方向
            console.log("碰到墙壁，改变方向");
        }

        // 检查是否被攻击
        if (selfCollider.tag === 'enemy6body' && otherCollider.tag === 'MaoQi') {
            const attackDirection = this.enemy6.getPlayerLastAttackDirection(); // 假设有一个方法获取玩家上次攻击的方向
            this.enemy6.changeState(new HurtState(this.enemy6, this, attackDirection));
        }
    }
    
    private isPlayerInRange(playerTransform: Transform): boolean {
        const distance = Math.sqrt(Math.pow(playerTransform.x - this.enemy6.gameObject.getBehaviour(Transform).x, 2) + 
                                   Math.pow(playerTransform.y - this.enemy6.gameObject.getBehaviour(Transform).y, 2));
...
export class ChaseState extends Enemy6State {
    ...
    update(duringTime: number) {
        ...
        if (playerTransform) {
            ...
            let moveX = Math.cos(direction) * this.enemy6.chaseSpeed;
            let moveY = Math.sin(direction) * this.enemy6.chaseSpeed;
            ...
            if (playerTransform.x > transform.x) {
                this.enemy6.enemy6Binding!.action = 'rightrun';
            } else {
                this.enemy6.enemy6Binding!.action = 'leftrun';
            }
        }

        // 检测是否进入攻击状态
        if (this.isPlayerClose(playerTransform)) {
            this.enemy6.changeState(new AttackState(this.enemy6));
        }

        // 检测是否玩家离开追逐范围
        if (!this.isPlayerInRange(playerTransform)) {
            this.enemy6.changeState(new PatrolState(this.enemy6));
        }
...
export class AttackState extends Enemy6State {
    ...
    update(duringTime: number) {
        ...
        if (playerTransform) {
            // 根据小怪和玩家的位置确定攻击方向
            if (playerTransform.x < transform.x) {
                this.enemy6.enemy6Binding!.action = 'leftattack';
            } else {
                this.enemy6.enemy6Binding!.action = 'rightattack';
            }
